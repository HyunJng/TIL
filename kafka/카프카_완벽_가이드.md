...
* 모든 Consumer은 고유한 그룹아이디 group.id를 가지는 Consumer Group에 소속되어야함
* 개별 Consumer Group내에서 여러 개의 Consumer들을 토픽 파티션 별로 분배됨
  * 보통 partition 개수와 consumer들의 개수를 맞춤

---
### subscribe, poll, commit 로직
![카프카완벽가이드_14.png](/kafka/images/%EC%B9%B4%ED%94%84%EC%B9%B4%EC%99%84%EB%B2%BD%EA%B0%80%EC%9D%B4%EB%93%9C_14.png)
* consumer이 만들어질 때 subscribe()를 통해 읽어들이려는 토픽의 메타정보를 가져옴
* poll()을 통해 주기적으로 브로커의 토픽 파티션에서 메시지를 가져옴
* 메시지를 성공적으로 가져왔으면 coommit을 통해서 __consumer_offsets 에 다음에 읽을 offset 위치를 기재함
  * __consumer_offsets : Broker의 내부토픽.

---
### KafkaConsumer의 주요 구성 요소
![카프카완벽가이드_15.png](/kafka/images/%EC%B9%B4%ED%94%84%EC%B9%B4%EC%99%84%EB%B2%BD%EA%B0%80%EC%9D%B4%EB%93%9C_15.png)
![카프카완벽가이드_16.png](/kafka/images/%EC%B9%B4%ED%94%84%EC%B9%B4%EC%99%84%EB%B2%BD%EA%B0%80%EC%9D%B4%EB%93%9C_16.png)
* Linked Queue에 데이터가 있으면 Fetcher는 데이터를 가져오고 반환하며 poll() 수행 완료
* Linked Queue에 데이터가 없으면 설정된 ms까지 Broker에 메시지 요청 후 poll() 수행 완료
* ConsumerNetworkClient는 비동기로 계속 브로커의 메시지를 가져와서 LinkedQueue에 저장(큐에 없으면!)
...
* 데이터가 전혀 없다 하면 최대 1000ms동안 데이터 Fetch를 브로커에 계속 수행하고 결과 반환

---
### Consumer Fetcher 관련 설정 파라미터
* fetch.min.bytes
  * Fetcher가 record들을 읽어들이는 최소 bytes. 
  * 브로커는 지정된 값 이상의 새로운 메시지가 쌓일 때까지 전송하지 않음. 
  * 기본은 1 bytes
* fetch.max.wait.ms
  * 브로커에 fetch.min.bytes 이상의 메시지가 쌓이지 않아도 최대 이 시간만큼 기다리고 보냄.
  * 기본은 500ms
  > `poll(1000ms)`에 설정하는 시간은 **레코드가 없으면** 1000ms까지 기다리는거고
  > `fetch.max.wait.ms`는 메시지가 있는데 기다리는 시간
* fetch.max.bytes
  * Fetcher가 한번에 가져올 수 있는 최대 데이터 bytes. 
  * 기본은 50MB
* max.partition.fetch.bytes
  * Fetcher가 파티션별 한번에 최대로 가져올 수 있는 bytes
  * partition이 10개고 이 설정이 10MB이라면 50MB가 가져와짐(but, fetch.max.bytes보다 적게)
* max.poll.records
...
#### 파라미터 설정 예시
```properties
fetch.min.bytes=16384 <<< 중요
fetch.max.wait.ms=500
fetch.max.bytes=52428800
max.partition.fetch.bytes=1024168 <<< 중요
max.poll.records=500
// java
KafkaConsumer.poll(1000)
```
1. 가져올 데이터가 한 건도 없으면 `poll()`인자 시간인 1000ms 대기 후 return
2. 가져와야할 데이터가 많은 경우 `max.partition.fetch.bytes`로 배치 크기 설정.
  그렇지 않으면 `fetch.min.bytes`로 배치 크기 설정(이건 강사님 뇌피셜)
  * 가장 최신의 offset 데이터를 가지고 있다면 `fetch.min.bytes`만큼 가져온 후 return.
    `fetch.min.bytes`만큼 쌓이지 않았다면 `fetch.max.wait.ms`만큼 기다린 후 return
    * `max.partition.fetch.bytes`에 도달하지 못해도 가장 최신의 offset에 도달하면 반환
  * 오랜 과거 offset 데이터를 가져온다면 최대 `max.partition.fetch.bytes`만큼 파티션에서 읽은 뒤 반환
    * 토픽에 파티션이 많아도 가져오는 데이터 량은 `fetch.max.bytes`로 제한
  * Fetcher가 Linked Queue에서 가져오는 레코드 개수는 `max.poll.records`로 제한

---
### Consumer의 `auto.offset.reset`
* Consumer이 Topic에 **처음 접속**하여 Message를 가져올 때 가장 오래된 처음 offset부터(earliest) 가져올 것인지,
  가장 최근인 마지막 offset 부터(latest) 가져올 것인지 설정한는 파라미터.
* default는 latest이다.
  * `auto.offset.reset=earliest` : **__consumer_offset이 없을 때** 처음 offset 부터 읽음
  * `auto.offset.reset=latest` : 마지막 offset 부터 읽음

> `__cumsumer_offset`은 consumer 그룹별, Topic의 개별 파티션별로 가지고 있음
* 동일 Consumer Group으로 Consumer이 새롭게 접속할 시 `__cumsumer_offset`에 있는 offset 정보를 기반으로 
  메시지를 가지고 오기 떄문에 ealiest로 설정해도 0번 offset부터 읽어들이지 않음
* Consumer Group의 Consumer가 모두 종료되어도 Consumer Group이 읽어들인 offset 정보는 기본 7일간 `__cumsumer_offset`을 보관(`offsets.retention.minutes`)
* 해당 Topic이 삭제되고 재 생성될 경우에는 해당 topic에 대한 Consumer Group의 offset 정보는 0으로 __consumer_offsets으로 기록됨(실무 절대 X)
