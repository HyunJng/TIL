# 2장. 테스트
## 1. "단위테스트"의 필수요소
1. 가능하면 작은 단위로 쪼개서 하나의 관심만 집중해야한다.
2. 테스트가 자동으로 수행되어야 한다.(데이터 생성, 결과 확인)
3. DB같은 외부 환경에 영향을 받지 말아야한다.
4. 테스트 순서가 바뀌어도 동일한 결과가 나와야한다.

---
## 2. 단위 테스트 생성 시 좋은 습관
* 항상 네거티브 테스트 먼저 만들라
* 먼저 설계를 테스트코드로 체계화하고 개발하라`TDD`

---
## 3. JUnit
### 3.1. Junit이란
* 테스트 **"프레임워크"**
* 실행 제어권을 갖고 있기 때문에 라이브러리가 아닌 프레임워크이다. 
...
---
### 3.2. JUnit 테스트 클래스 실행 순서
1. 테스트 클래스에서 `@Test`가 붙고 `public void` 형 메소드를 모두 찾는다.
2. 테스트 클래스의 오브젝트를 하나 만든다
3. `@Before` 가 붙은 메소드가 있으면 실행한다.
4. `@Test`가 붙은 메소드 하나를 호출하고 테스트 결과를 저장해둔다.
5. `@After`이 붙은 메소드가 있으면 실행한다.
6. 나머지 테스트 메소드에 대해 2 ~ 5번을 반복한다.
7. 모든 테스트 결과를 종합해서 돌려준다.
 
> **매번 테스트 오브젝트를 만드는 이유**<br/>
> 테스트가 서로 영향을 주지 않고 독립적으로 실행됨을 확실히 보장해주기 위함
...
---
## 5. Spring과 Junit
### 5.1. 아무 조치 없이 Spring과 Junit을 함께 사용할 때 문제점
...
---
### 5.2. Spring과 Junit을 함께 사용하기 위한 조치
...
---
### 7.3. `@ContextConfiguration` vs `@SpringApplicaitonCongifguation`vs `@SpringBootTest`
...
---
### 9.3. `@SpringBootTest + @AutoConfigureMockMvc`
