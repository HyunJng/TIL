...
## 전략패턴
### 문제상황
- 모든 클래스에서 상속하는 메서드만 interface로 구현해두어야한다
    - ex) `Duck.class`에 `fly()`라는 메서드를 생성하면 고무오리도 날 수 있게 됨
- 그렇다고 `Flyable`이라는 인터페이스로 만들면 코드를 재사용할 수 없다

> 오리의 행동을 메서드로 구현하는게 아니라 다른 "클래스"에 역할을 위임한다

### 해결
* 변경될 수 있는 부분은 인터페이스로 통째로 분리 
* 해당 인터페이스의 구현체를 클라이언트에게 주입받아 사용한다(Composition, DI)
```
